param(
    ...
    [Parameter(Mandatory = $false)]
    [string] $EmailSubjectPrefix = $(if ($env:EMAIL_SUBJECT_PREFIX) { $env:EMAIL_SUBJECT_PREFIX } else { '[Security notification] Device registered:' })
)
...
$deviceName = Get-FirstDetailValue -Details $details -Name 'DeviceName'
$deviceId = Get-FirstDetailValue -Details $details -Name 'DeviceId' -DefaultValue 'Not recorded'
$joinType = Get-FirstDetailValue -Details $details -Name 'JoinType' -DefaultValue 'Not recorded'
$operatingSystem = Get-FirstDetailValue -Details $details -Name 'OperatingSystem' -DefaultValue 'Not recorded'
$osVersion = Get-FirstDetailValue -Details $details -Name 'OSVersion' -DefaultValue 'Not recorded'
...
$message = @{
    message = @{
        subject = "$EmailSubjectPrefix $deviceName"
        body = @{
            contentType = 'Text'
            content = @"
The following device was registered with your account.
...
If you registered this device, no action is required. If you did not, contact $HelpdeskContact immediately.
"@
        }
        toRecipients = @(
            @{
